...
class Question: #This Class is used specifically for the variables to isolate the correct answers, the mulitchoice buttons, and the questions
    #class construction for the  initialisation, that contains how the variable of question and correct_answer will be calculated in class QuizBrain
    def __init__(self, question: str, correct_answer: str, choices: list):
        self.question_text = question
        self.correct_answer = correct_answer
        self.choices = choices

class QuizBrain: #class QuizBrain that contains all functions related to how the quiz processes work (switching questions, checking answers, recording score)
    def __init__(self, questions): #Initialising QuizBrain by setting the question number and user score to 0, and to set current question to None, to intialise the answer checking code
        self.question_no = 0
        self.score = 0
...
class QuizInterface: #class QuizInterface used to create the interface for the quiz and home screens, creating the window, buttons, text, etc
    #Initilisation of class by setting all variables and methods for window sizes and titles, canvases, and looping the window
    def __init__(self, quiz_methods: QuizBrain) -> None:
        ...
        # Display title as method, with parameter of the quiz window
        self.display_title(self.quizwindow)

        # Create a canvas for question text, and display question as method
        ...
        # Display four options as radio buttons, and option display as method
        ...
        # Next, home and quit buttons as method
        self.quizbuttons()

        #loop of the quizwindow
    ...
    def homebuttons(self):
        #To show quit, info, start buttons using screendwidth and screenheight methods
    ...
    #function to instruct what the quit button does, using quiz.window method
    def quit(self):
        self.quizwindow.destroy()
    ...
    def display_question(self):
        #To display the question using next_question method
...
class userUI(): #class to put all functions and methods related to the user interface and user inputs at the beginning of quiz
    def __init__(self): #initialising class by defining the methods used in class (to display title, user input, start buttoms, and to loop window), as well as intilaising user window as tk widget
        self.userwindow = Tk()
    ...
    def display_title(self, windowname):
        # Function to display title of window
    ...
    def display_entries(self): #Function to display the name entry box using the name_entries method with the parameter of the user window 
    ...
    def get_name(self): #function that checks whether name is valid input or not, and if valid, continues to quiz and appends user name to SQL database
        if self.name_entry.get().replace(' ', '').isalpha() == True: # If the entered name (without spaces) is without numbers, continue, else give an error message
            ...
            pass
        else: #If non valid name input detected (non alphabet characters input), window is updated and screen displays error message before allowing user to reinput name
    ...
    def age_entries(self, windowname):
        # Function that Hides widgets from previous screen, rewords label, and creates new button with different command
        maxwidth = windowname.winfo_screenwidth()
    ...
    def get_age(self):#function that checks whether age is valid input or not (using try except loop), and if valid, continues to quiz and appends user name to SQL database
        try:
            if int(self.amount_entry.get().replace(' ', '')) <= 100 and int(self.amount_entry.get().replace(' ', '')) > 0: #If the entered value is between 0 and 50, continue, else give error message
                ...
                time.sleep(1)
                self.start_quiz()
            else: #else statement used for if an integer less than 0 or greater than 100 is entered, as those ages are almost impossible to use quiz, and thus are invalid
        ...
        except ValueError: #except ValueError for if an age input from user is detetced as invalid (an alphabetic character is input), which displays error message and updates screen for user to reinput
...
#parameters variable, where amount is set to 8 for 8 questions, and type is set to multiple, for multiple choice answers
parameters = {
    "amount": 8,
    "type": "multiple"
}

#response variable, used to get data from opentdb, and params is set to the parameter specifications of 8, and multiple
response = requests.get(url="https://opentdb.com/api.php", params=parameters)
question_data = response.json()["results"] #question_data variable to be set to the question and answer data from opentdb by using the response json method set as results

"""code to take question_data and set choices and question_bank to empty list, that gains the answer and incorrect answer for each individual question from database, and stores as #question, correct_answer, and incorrect_answers respectively"""
question_bank = []
...
userUI() #calling userUI function so it runs the user input screen first